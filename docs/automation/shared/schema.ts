...
export const waForms = pgTable("wa_forms", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  name: text("name").notNull(),
  ...
  metaId: text("meta_id"), // ID from WhatsApp/Meta
  createdBy: varchar("created_by").references(() => users.id),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const insertWaFormSchema = createInsertSchema(waForms).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type WaForm = typeof waForms.$inferSelect;
export type InsertWaForm = z.infer<typeof insertWaFormSchema>;
...
export const waFormsRelations = relations(waForms, ({ one }) => ({
  author: one(users, {
    fields: [waForms.createdBy],
    references: [users.id],
  }),
}));
