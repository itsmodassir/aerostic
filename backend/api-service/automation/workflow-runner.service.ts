...
import {
  ...
  NodeExecutionStatus,
} from "./entities/workflow-execution-log.entity";
...
export class WorkflowRunnerService {
  ...
  async executeWorkflow(
    ...
    const contactId =
      initialContext.trigger?.contactId || initialContext.contactId;
    if (contactId) {
      const memoryRecords = await this.memoryRepo.find({
        where: { tenantId, contactId },
      });
      initialContext.memory = memoryRecords.reduce(
    ...
    try {
      ...
      const triggerNode = workflow.nodes.find(
        (n) =>
          n.type === "trigger" || n.type === "webhook" || n.type === "manual",
      );
  ...
  private async executeNode(
    ...
    try {
      ...
      // Emit Success Event
      this.automationGateway.emitExecutionEvent(
        execution.id,
        "node_completed",
        {
          nodeId: node.id,
      ...
      if (node.type === "condition" && result.branch) {
        filteredEdges = edges.filter(
          (e) => e.sourceHandle === result.branch || !e.sourceHandle,
        );
      }
