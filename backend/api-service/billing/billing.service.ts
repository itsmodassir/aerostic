...
export class BillingService {
  ...
  constructor(
    ...
    private razorpayEventRepo: Repository<RazorpayEvent>,
  ...
  async handleWebhookEvent(body: any): Promise<{ received: boolean }> {
    ...
    const existingEvent = await this.razorpayEventRepo.findOne({
      where: { eventId: body.id },
    });

    if (existingEvent) {
      this.logger.log(`Event ${body.id} already processed`);
      return { received: true };
    }

    // 2. Persist Event immediately
    await this.razorpayEventRepo.save(
      this.razorpayEventRepo.create({
        eventId: body.id,
        event: eventName,
    ...
    try {
      ...
      await this.razorpayEventRepo.update(
  ...
  async validateApiKey(rawKey: string): Promise<ApiKey | null> {
    ...
    const apiKey = await this.apiKeyRepo.findOne({
      where: { keyHash, isActive: true, killSwitchActive: false },
    });
  ...
  async getUsageMetrics(tenantId: string) {
    ...
    const messages =
      metrics.find((m) => m.metric === "messages_sent")?.value || 0;
    const aiCredits =
      metrics.find((m) => m.metric === "ai_credits")?.value || 0;
    const broadcasts =
      metrics.find((m) => m.metric === "broadcasts_sent")?.value || 0;
