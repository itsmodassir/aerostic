...
export class AutomationService {
    ...
    async evaluate(tenantId: string, from: string, messageBody: string): Promise<boolean> {
        ...
        for (const rule of rules) {
            ...
            if (rule.trigger === 'keyword') {
                if (rule.condition === 'contains' && lowerMsg.includes(lowerKey)) match = true;
    ...
    private async executeAction(tenantId: string, to: string, rule: AutomationRule) {
