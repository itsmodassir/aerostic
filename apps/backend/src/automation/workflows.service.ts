...
export class WorkflowsService {
    ...
    async executeTrigger(tenantId: string, triggerType: string, context: any) {
        ...
        for (const workflow of activeWorkflows) {
            ...
            if (context.conversationId) {
                const conversation = await this.messagesService.getConversation(context.conversationId);
                if (conversation && conversation.status === 'agent_handoff') {
                    this.logger.log(`Skipping workflow trigger for ${context.conversationId} (Agent Handoff)`);
                    continue;
                }
            }
            ...
            try {
                ...
                switch (nextNode.type) {
                    ...
                    case 'api_request':
                        ...
                        break;
                    case 'google_drive':
                        await this.handleGoogleDriveNode(workflow.tenantId, nextNode, context);
                        break;
                }
    ...
    private async handleAiNode(workflow: Workflow, node: any, context: any) {
        ...
        // Tool Discovery
        const tools = [];
        const incomingEdges = workflow.edges.filter(e => e.target === node.id && e.targetHandle === 'tool-target');
        ...
        if (tools.length > 0) {
            ...
            const response = await this.aiService.runAgent(tenantId, context.messageBody, tools, systemPrompt || 'You are a helpful agent.');
            ...
            this.logger.log(`Forwarding to AI Service (Legacy): ${tenantId}, Persona: ${persona}`);
    ...
    private async handleContactNode(tenantId: string, node: any, context: any) {
        ...
        if (operation === 'get') {
            const contact = await this.contactsService.findOne(context.contactId, tenantId);
            if (contact) {
    ...
    private async handleChatNode(tenantId: string, node: any, context: any) {
        ...
        if (context.conversationId) {
            await this.messagesService.setConversationStatus(context.conversationId, 'agent_handoff');
        }
    }
    ...
    private async handleApiNode(tenantId: string, node: any, context: any) {
        ...
        try {
            ...
            const parsedBody = body && method !== 'GET' ? JSON.parse(body) : undefined;
    ...
    private async handleGoogleDriveNode(tenantId: string, node: any, context: any) {
        const { operation, fileId, fileName, fileContent, mimeType, variableName } = node.data;
        ...
        try {
            ...
            if (operation === 'upload') {
                ...
                result = await this.googleService.uploadFile(tenantId, accessToken, fileContent || 'Mock Content', fileName || 'upload.txt', mimeType || 'text/plain');
            } else if (operation === 'read') {
                result = await this.googleService.readFile(tenantId, accessToken, fileId);
