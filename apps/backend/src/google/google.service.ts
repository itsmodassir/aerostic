...
export class GoogleService {
    ...
    async uploadFile(tenantId: string, accessToken: string, content: string, fileName: string, mimeType: string) {
    ...
    async getAccessToken(tenantId: string): Promise<string | null> {
        const account = await this.googleAccountRepo.findOne({ where: { tenantId } });
    ...
    getTools(tenantId: string): Tool[] {
        return [
            ...
            },
            {
                ...
                description: 'Read the content of a text file from Google Drive given its ID.',
                parameters: {
                    type: 'object',
                    properties: {
            ...
            },
            {
                ...
                execute: async (args: any) => {
                    const token = await this.getAccessToken(tenantId);
                    if (!token) throw new Error('Google Account not connected');
                    return this.uploadFile(tenantId, token, args.content, args.fileName, 'text/plain');
                }
            }
        ];
    }
