...
export class MetaTokenService {
    ...
    async handleTokenRefresh() {
        this.logger.log('Running automated WhatsApp token refresh job (Multi-Tenant)...');
        ...
        if (!appId || !appSecret) {
            this.logger.error('Meta App ID or Secret missing in configuration. Skipping refresh.');
            return;
        }
        ...
        for (const account of accountsToRefresh) {
            try {
                ...
                this.logger.log(`Refreshing token for account ${account.phoneNumberId} (Tenant: ${account.tenantId})...`);
                ...
                account.tokenExpiresAt = new Date(Date.now() + (longTokenData.expires_in || 5184000) * 1000);
                ...
                this.logger.log(`Successfully refreshed token for ${account.phoneNumberId}. New expiry: ${account.tokenExpiresAt}`);
            } catch (error: any) {
                this.logger.error(`Failed to refresh token for ${account.phoneNumberId}: ${error.message}`);
            }
        }
    }
}
