...
export class AddSlugToTenant1770717221653 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "tenants" ADD "slug" character varying`);

        // 2. Populate existing rows with a safe slug
        await queryRunner.query(`UPDATE "tenants" SET "slug" = lower(regexp_replace("name", '[^a-zA-Z0-9]+', '-', 'g')) WHERE "slug" IS NULL`);
