...
describe('AdminController (e2e)', () => {
    ...
    const mockAdminConfigService = { getConfig: jest.fn(), setConfig: jest.fn(), deleteConfig: jest.fn() };
    const mockAdminTenantService = { getAllTenants: jest.fn(), getAllTenantsSummary: jest.fn(), updateUserPlan: jest.fn() };
    const mockAdminBillingService = { getAllApiKeys: jest.fn(), getAllWebhooks: jest.fn(), getBillingStats: jest.fn() };
    const mockAdminHealthService = { checkSystemHealth: jest.fn(), getSystemLogs: jest.fn() };
    const mockAdminAnalyticsService = { getDashboardStats: jest.fn(), getAnalyticsTrends: jest.fn(), getSystemAlerts: jest.fn() };
    const mockAdminService = { getAllAccounts: jest.fn(), rotateSystemTokens: jest.fn(), getAllMessages: jest.fn() };
    ...
    beforeAll(async () => {
        const moduleFixture: TestingModule = await Test.createTestingModule({
            ...
            .overrideProvider(AdminConfigService).useValue(mockAdminConfigService)
            .overrideProvider(AdminTenantService).useValue(mockAdminTenantService)
            .overrideProvider(AdminBillingService).useValue(mockAdminBillingService)
            .overrideProvider(AdminHealthService).useValue(mockAdminHealthService)
            .overrideProvider(AdminAnalyticsService).useValue(mockAdminAnalyticsService)
            .overrideProvider(AdminAnalyticsService).useValue(mockAdminAnalyticsService)
            .overrideProvider(AdminService).useValue(mockAdminService)
            .overrideProvider(BillingService).useValue(mockBillingService)
            .overrideProvider(RazorpayService).useValue(mockRazorpayService)
